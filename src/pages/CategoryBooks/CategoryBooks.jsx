    ...
    useEffect(() => {
        const fetchCategoryBooks = async () => {
            try {
                ...
                const res = await axiosInstance.get(`/api/allBooks?category=${category}&page=${currentPage}&limit=${itemsPerPage}`);
        ...
        <div className="flex justify-between items-center bg-[#e6eff2] dark:bg-[#19343d] sm:py-6 py-12">
            ...
            <div className="flex flex-col items-center justify-center text-center mx-auto sm:py-0 py-3">
                ...
                <div className="flex items-center gap-1 text-[10px] sm:text-xs md:text-sm font-normal text-gray-600 dark:text-slate-400 sm:mt-1">
                    <Link to="/">Home</Link>
                    <IoIosArrowForward />
                    <span className='text-orange-500 dark:text-orange-300'>Category Books</span>
                </div>
            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:px-0 sm:px-16">
                            {filteredBooks.map((book) => (
                                <div key={book._id} className="relative group rounded-xl">
                                    ...
                                    <p className='absolute top-9 left-4 text-[10px] bg-[var(--color-dark-secondary)] text-white font-semibold py-1 px-[6px] rounded'>{book.category}</p>
                                    <p className='absolute top-3 left-4 text-[10px] bg-orange-500 text-white font-semibold py-1 px-[6px] rounded'>Stock: {book.quantity}</p>
                                ...
                                <tbody>
                                    {filteredBooks.map((book, index) => (
                                        <tr key={book._id} className="text-center">
                                            ...
                                            <td className="border-b-2 text-gray-300  dark:border-[#374151] lg:text-base md:text-sm text-xs font-semibold border-gray-200 px-4 py-2">Stock: <span className='text-orange-400'>{book.quantity}</span></td>
                                            <td className="border-b-2 border-gray-200 dark:border-[#374151] px-4 py-2">
